...
class ResampleLoss(torch.nn.Module):
    def __init__(
        self,
        use_sigmoid=True,
        partial=False,
        ...
        CB_loss=dict(
            CB_beta=0.9, CB_mode="average_w"  # 'by_class', 'average_n', 'average_w', 'min_n'
        ),
    ...
    def forward(
        self, cls_score, label, weight=None, avg_factor=None, reduction_override=None, **kwargs
    ):
        ...
        if self.focal:
            logpt = self.cls_criterion(
                cls_score.clone(), label, weight=None, reduction="none", avg_factor=avg_factor
            )
            ...
            loss = (
                alpha_t * ((1 - pt) ** self.gamma) * wtloss
            )  ####################### balance_param should be a tensor
    ...
    def CB_weight(self, gt_labels):
        if "by_class" in self.CB_mode:
            ...
            avg_n = torch.sum(gt_labels * self.class_freq, dim=1, keepdim=True) / torch.sum(
                gt_labels, dim=1, keepdim=True
            )
            weight = (
                torch.tensor((1 - self.CB_beta)).cuda()
            ...
            weight = torch.sum(gt_labels * weight_, dim=1, keepdim=True) / torch.sum(
                gt_labels, dim=1, keepdim=True
            )
        elif "min_n" in self.CB_mode:
            min_n, _ = torch.min(
                gt_labels * self.class_freq + (1 - gt_labels) * 100000, dim=1, keepdim=True
            )
            weight = (
                torch.tensor((1 - self.CB_beta)).cuda()
