...
class Model(nn.Module):
    def __init__(self, model_name_or_path: str, num_labels:int, dropout_rate=0.3, output_length=384):
        ...
        #self.l2 = torch.nn.BatchNorm1d(output_length)
...
class Transformer(pl.LightningModule):
    def __init__(self,
        ...
        self.training_loader = self.get_loaders (train_dataset,
                                    train_params,
                                    tagname_to_tagid,
                                    self.tokenizer,
                                    max_len
                                    )
        self.val_loader = self.get_loaders (val_dataset,
                                    val_params,
                                    tagname_to_tagid,
                                    self.tokenizer,
                                    max_len
    ...
    def custom_predict(self, validation_dataset, return_all=False):
        validation_loader = self.get_loaders (validation_dataset,
                                                self.val_params,
        ...
        if return_all:
            return np.array(predictions), y_true_final, indexes_final, zip(indexes, logit_predictions)
    ...
    def get_results_pillar_from_subpillar (self, preds_val_all, y_true):
        ...
        list_tags_pillars = sorted(list(set([tags.split('->')[0] for tags in list_tags_subpillars])))
        pillars_tagname_to_tagid = {tag:i for i, tag in enumerate(list(sorted(list_tags_pillars)))}

        n_subpillars_per_pillar = [len(list(filter(lambda x: pillar in x, list_tags_subpillars))) for pillar in list_tags_pillars]
    ...
    def custom_eval(self, 
        ...
        print('ratio of evaluated sentences (%):', len(y_true) * 100 / validation_dataset.shape[0])
