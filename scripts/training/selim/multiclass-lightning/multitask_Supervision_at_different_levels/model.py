...
class Transformer(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.val_targets = val_dataset['target']
        ...
        if len(self.ids_each_level) == 1:
            ...
            for i_th_level in range(len(self.ids_each_level)):
                ...
                """mask_ids_not_neg_example = [
                    bool(int(torch.sum(one_row))) for one_row in targets_one_level
                ]
                
                loss_one_level = self.Focal_loss(
                    outputs_i_th_level[mask_ids_not_neg_example], 
                    targets_one_level[mask_ids_not_neg_example]
                    )"""

                tot_loss += loss_one_level
    ...
    def get_first_level_ids(self):
        ...
        if np.all(["->" in name for name in all_names]):
            ...
            if nb_first_levels > 4:
                ...
                self.hidden_layer_ids = [item - nb_first_levels for item in sorted_args_lengths]
