...
def get_full_list_entries(df, tag: str):
    """
...
def get_hypotheses(targets: List[str]) -> List[str]:

    names = [one_target.split("->")[-1] for one_target in targets]
    hypothesis_base = "{} is discussed."

    hypotheses = [hypothesis_base.format(one_target_name) for one_target_name in names]
    return hypotheses
...
def get_tag_id_to_layer_id(ids_each_level):
    ...
    list_id = 0
    tag_to_list = {}
    for id_list in flatten(ids_each_level):
