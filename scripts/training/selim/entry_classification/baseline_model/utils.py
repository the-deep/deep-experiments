...
def custom_stratified_train_test_split(df, ratios):
    ...
    for entry in unique_entries:
        ...
        if n_ids_entry <= 1:
            train_ids_entry = ids_entry
            val_ids_entry = []
        elif n_ids_entry <= 10:
            train_ids_entry = random.sample(ids_entry, n_ids_entry - 1)
            val_ids_entry = list(set(ids_entry) - set(train_ids_entry))
        else:
            train_ids_entry = random.sample(
                ids_entry, int(n_ids_entry * ratios["train"])
            )
            val_ids_entry = list(set(ids_entry) - set(train_ids_entry))
...
def augment_numbers_sentences(df):
    ...
    def augment_sent(txt: str):
        augmented_txt = " ".join(
            [
                "".join([str(change_nb(c)) if c.isalnum() else c for c in word])
...
def get_negative_positive_examples(df: pd.DataFrame):
    ...
    all_negative_ids = (
        df_bis[
            ...
            & df_bis.target.apply(lambda x: len(x) == 0)
        ]
        .entry_id.unique()
        .tolist()
    )
    ...
    all_positive_ids = (
        df_bis[df_bis.target.apply(lambda x: len(x) > 0)].entry_id.unique().tolist()
    )
