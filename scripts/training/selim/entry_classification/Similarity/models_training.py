...
from TransformerModel import TrainingTransformer, TrainingMLP, LoggedClassificationModel

from pytorch_lightning.callbacks import ModelCheckpoint
from pytorch_lightning.utilities import rank_zero_only

from utils import (
    _custom_stratified_splitting,
...
def train_model(
    ...
    #     n_tokens = _get_n_tokens(trainable_data.en.tolist())
    #     trainable_data = trainable_data.iloc[
    #         (n_tokens <= int(1.1 * max_len)) & (n_tokens > 10)
    #     ]

    # Stratified splitting project-wise
    train_ids, val_ids, test_ids = _custom_stratified_splitting(
        trainable_data.project_id.tolist()
    )
    train_df = _create_df_with_translations(
        trainable_data[trainable_data.project_id.isin(train_ids)]
    )
    # train_mlp_df = _create_df_with_translations(
    #     trainable_data[trainable_data.project_id.isin(train_mlp_ids)]
    # )
    val_df = _create_df_with_translations(
        trainable_data[trainable_data.project_id.isin(val_ids)]
    )
    test_df = trainable_data[trainable_data.project_id.isin(test_ids)]
    test_df["excerpt"] = test_df.apply(lambda x: x[x["original_language"]], axis=1)

    targets_list = trainable_data["target"].tolist()
    ...
    # train_dataset = _create_df_with_translations(train_dataset)
    ...
    checkpoint_callback = MyModelCheckpoint(monitor="val_loss", mode="min")
    # MyModelCheckpoint(
    #     filename=MODEL_NAME, **checkpoint_callback_params
    # )

    llm_model_trainer = pl.Trainer(
        logger=None,
    ...
    trained_llm_model = TrainingTransformer(
    ...
    train_dataloader = trained_llm_model.get_loaders(train_df, train_params)
    # train_mlp_dataloader = trained_llm_model.get_loaders(train_mlp_df, train_params)
    val_dataloader = trained_llm_model.get_loaders(val_df, val_params)

    llm_model_trainer.fit(trained_llm_model, train_dataloader, val_dataloader)
    trained_llm_model.eval()
    trained_llm_model.freeze()

    trained_mlp_model = TrainingMLP(trained_llm_model, train_df)
    mlp_model_trainer.fit(trained_mlp_model, train_dataloader, val_dataloader)

    # create new data structure, containing only needed information for deployment used for logging models
    returned_model = LoggedClassificationModel(trained_mlp_model)
