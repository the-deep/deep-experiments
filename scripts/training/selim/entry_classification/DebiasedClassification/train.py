...
if __name__ == "__main__":
    ...
    with mlflow.start_run():
        ...
        # conterfactual test
        COUNTERFACTUAL_DATA_FOLDER = os.path.join(
            args.output_data_dir, "counterfactual_test"
        )
        os.makedirs(COUNTERFACTUAL_DATA_FOLDER, exist_ok=True)
        test_df = _get_genders_countries(test_df)
        counterfactual_test_dfs = _initialize_test_counterfactual(
            test_df, args.max_len, tokenizer
        )

        for bias_type, bias_df in counterfactual_test_dfs.items():
            bias_df.to_csv(
                os.path.join(COUNTERFACTUAL_DATA_FOLDER, f"{bias_type}.csv"), index=None
            )  # TODO: add model bias assesment
        ...
        torch.save(transformer_model, f"{Transformer_model_path}.pt")
