...
def _custom_stratified_splitting(
    ...
    n_leads = len(project_ids)
    # n_train_mlp_leads = int(n_leads * ratios["train_mlp"])
    n_val_leads = int(n_leads * ratios["val"])
    n_test_leads = int(n_leads * ratios["test"])

    project_counts = dict(Counter(project_ids))
    project_counts = dict(sorted(project_counts.items(), key=lambda x: x[1]))

    for i, (proj_id, proj_count) in enumerate(project_counts.items()):
        if (i % 3) == 0:  # when 1: val
            if val_len + proj_count <= n_val_leads:
                val_ids.append(proj_id)
                val_len += proj_count
            else:
                train_ids.append(proj_id)
        elif (i % 3) == 1:
            if test_len + proj_count <= n_test_leads:
                test_ids.append(proj_id)
                test_len += proj_count
            else:
                train_ids.append(proj_id)
        # elif (i % 4) == 2:
        #     if train_mlp_len + proj_count <= n_train_mlp_leads:
        #         train_mlp_ids.append(proj_id)
        #         train_llm_ids.append(proj_id)
        #         train_mlp_len += proj_count
        #     else:
        #         train_llm_ids.append(proj_id)
        else:
            train_ids.append(proj_id)
...
def _preprocess_df(df: pd.DataFrame, min_entries_per_proj: int = None) -> pd.DataFrame:
    """
    ...
    #     (all_data.project_id.isin(projects_for_out_of_context_testing))
    #     & (all_data.original_language.isin(languages))
    # ]
    # out_of_context_test_data["excerpt"] = out_of_context_test_data.apply(
    ...
    return (all_data, projects_list_per_tag)
...
def get_tag_id_to_layer_id(ids_each_level):
    tag_id = 0
    tag_to_list = {}
    for i, ids_list in enumerate(ids_each_level):
        for ids_sublist in ids_list:
            for _ in range(len(ids_sublist)):
                tag_to_list.update({tag_id: i})
                tag_id += 1
    return tag_to_list
...
def _get_n_tokens(text: List[str]) -> np.ndarray:
    """
    get number of tokens after tokeniziation for excerpts.
    """
    lengths = np.array([len(sent.split(" ")) for sent in text])
    return lengths
...
#     if sample_data:
#         train_val_df = train_val_df.sample(n=1_000)
