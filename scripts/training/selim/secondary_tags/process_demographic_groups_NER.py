...
def get_list(List):
    list_words = list(
        set(
            chain.from_iterable(
                [word.lemma_names(lang) for lang in languages for syn in List for word in syn]
            )
        )
    )
...
def get_demographic_groups(import_data, data_path: str = None, data: pd.DataFrame = None):
    ...
    demo_groups_data["predicted_tag"] = demo_groups_data["excerpt"].apply(
        lambda x: return_demographic_groups_one_sent(x)
    )
