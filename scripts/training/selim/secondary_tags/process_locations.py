...
not_equal_list = ['dc','no','south','north','west','east','Afrin','St','unknown','Sites','',
                  'north west', 'north east', 'Northwest', 'northeast', 'centre']
not_equal_list_up = [name.upper() for name in not_equal_list]

#If string IN prediction => omit all prediction
wrong_locations_list = ['école', 'éducation', 'insécurité','adolescent', 'Svp',
                        'Government','cluster', 'shocking', 'property','housing','feedback','mechanism',
                        'survey','conduct','song','import','fight','song','household','individ','broke','chairm',
                        'poor','borderline','accept','flood',
                        'nutrition', 'water','sanitat','people','hygiene','detail','report','world', 'complain',
                        'unama', 'covid', 'gbv']
...
def get_place (text,
    ...
    for item in list_items:
        ...
        final_item = final_item.replace("’s",'')\
                                .replace("'s",'')\
                                .replace("unknown",'')\
    ...
    else: return list(np.unique(cleaned_list))
...
def postprocess_predictions(one_lead_data:pd.DataFrame):
    ...
    empty_tags_df_lead.loc[:,'predicted_tag'] =\
            empty_tags_df_lead.loc[:,'predicted_tag'].apply(lambda x: [max_tag])
    
    filled_tags_df_lead['predicted_tag'].apply(lambda x: x.append(max_tag))
    filled_tags_df_lead['predicted_tag'] =\
            filled_tags_df_lead['predicted_tag'].apply(lambda x: np.unique(x))

    return pd.concat([empty_tags_df_lead, filled_tags_df_lead])
        

def get_locations (import_data, data_path:str=None, data:pd.DataFrame=None, augment_predictions=False):
    if import_data:
        geo_location_data = pd.read_csv(data_path, index_col=0)[['excerpt']].dropna()
    else:
        geo_location_data = data[['excerpt', 'tag_value']].dropna()

    geo_location_data['predicted_tag'] =\
         geo_location_data['excerpt'].apply(lambda x: get_place (x, augment_predictions=augment_predictions))
