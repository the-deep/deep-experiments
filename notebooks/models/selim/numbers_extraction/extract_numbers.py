...
def get_numbers(df: pd.DataFrame, min_ratio: float = 0.7):
    ...
    for one_pop, related_tag in kept_tags.items():
        ...
        one_pop_df = one_pop_df.sort_values(
            by="confidence", ascending=False, inplace=False
        )
        one_pop_df = one_pop_df[
            (one_pop_df.confidence > min_ratio) & (one_pop_df.answer.apply(has_numbers))
        ]
        ...
        if len(one_pop_df) > 0:
            ...
            returned_answer = (
                one_pop_df.groupby("answer", as_index=False)["confidence"]
                .apply(sum)
