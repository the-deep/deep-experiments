...
online-test-env-build:
	docker build . -f docker/Dockerfile.online_test_env -t online_test_env --no-cache --platform=linux/amd64

online-test-env-build-arm:
	docker build . -f docker/Dockerfile.online_test_env -t online_test_env-m1 --no-cache

online-test-env-deploy:
	aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 961104659532.dkr.ecr.us-east-1.amazonaws.com
	docker tag online_test_env 961104659532.dkr.ecr.us-east-1.amazonaws.com/online_test_env
	docker push 961104659532.dkr.ecr.us-east-1.amazonaws.com/online_test_env
